...
void DngDecoder::dropUnsuportedChunks(std::vector<const TiffIFD*>* data) {
  for (auto i = data->begin(); i != data->end();) {
    ...
    // Check photometric interpretation to filter out non-image SubIFDs
    // (e.g., Semantic Mask from DNG 1.6+)
    // Only specifically reject Semantic Mask, allow other photometric interpretations
    if (ifd->hasEntry(TiffTag::PHOTOMETRICINTERPRETATION)) {
      const uint32_t photometric =
          ifd->getEntry(TiffTag::PHOTOMETRICINTERPRETATION)->getU32();

      // 52527 = Semantic Mask (DNG 1.6+, "CD2F.H" per Adobe DNG 1.7.1 spec)
      // This is Apple's ML segmentation mask, not pixel data
      if (photometric == 52527) {
        i = data->erase(i);
        continue;
      }
    }
